...
function valueForHeader(data, header) {
  const keys = {
    ...
    "Current AI Uses": "uses", "AI Assistants": "assistants", "Paid AI Assistants": "paidAssistants", "Other AI Tools": "tools",
...
function reportHtml(data) {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><style>@page{size:Letter;margin:0}*{box-sizing:border-box}body{margin:0;padding:34px 42px;color:#1d2420;font:10px Arial,sans-serif}header{display:flex;align-items:center;justify-content:space-between;border-bottom:3px solid #4f805b;padding-bottom:16px;margin-bottom:22px}header img{max-width:190px;max-height:60px}h1{margin:0;color:#1d2420;font-size:23px}h1 span{display:block;color:#4f805b;font-size:11px;font-weight:normal;letter-spacing:1px;text-transform:uppercase;margin-top:4px}.date{color:#68716b;font-size:9px}.identity{display:grid;grid-template-columns:1fr 1fr;gap:5px 28px;padding:13px 15px;background:#f4f5f2;border-left:4px solid #4f805b;margin-bottom:19px}.identity b{display:block;color:#68716b;font-size:8px;text-transform:uppercase;letter-spacing:.6px;margin-bottom:3px}.identity p,.item p{margin:0;line-height:1.35;white-space:pre-wrap;word-wrap:break-word}.grid{display:grid;grid-template-columns:1fr 1fr;gap:14px 27px}.item{border-bottom:1px solid #d5dbd5;padding-bottom:8px;min-height:38px}.item b{display:block;color:#4f805b;font-size:8px;text-transform:uppercase;letter-spacing:.6px;margin-bottom:4px}.wide{grid-column:1/-1}.calendar-link{margin-top:18px;padding-top:8px;border-top:1px solid #d5dbd5;font-size:9px}.calendar-link a{color:#4f805b}footer{position:fixed;bottom:22px;left:42px;right:42px;border-top:1px solid #d5dbd5;padding-top:8px;color:#68716b;font-size:8px}</style></head><body><header>${logo ? `<img src="${logo}" alt="School logo">` : "<h1>Shelton<span>AI profile</span></h1>"}<div class="date">TEACHER AI PROFILE<br>${escapeHtml(formatValue(data.submittedAt))}</div></header><div class="identity">${section("Name", data.name)}${section("Email", data.email)}${section("Teaching / Role", data.role)}${section("Subjects / Roles", data.subjects)}${section("Years in Education", data.years)}${section("AI Frequency", data.frequency)}</div><div class="grid">${section("Current AI Uses", data.uses)}${section("AI Assistants", data.assistants)}${section("Paid AI Assistants", data.paidAssistants)}${section("Other AI Tools", data.tools)}${section("AI Perspective", data.mindset)}${section("Useful AI Goals", data.goals)}${section("Other Tool", data.otherTool)}${section("Other Goal", data.otherGoal)}${section("What They Wish To Learn", data.wish)}${section("Time-Saving Opportunity", data.painPoint)}</div><div class="calendar-link"><b>Schedule a conversation:</b> <a href="${escapeHtml(CALENDAR_URL)}">${escapeHtml(CALENDAR_URL)}</a></div><footer>This profile is a deterministic summary of the teacher's responses. It is not an assessment or an AI score.</footer></body></html>`;
}
